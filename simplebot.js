...
(function() {
  ...
  slapCommand = (function(_super) {

    __extends(slapCommand, _super);

    function slapCommand() {
      return slapCommand.__super__.constructor.apply(this, arguments);
    }

    slapCommand.prototype.init = function() {
      this.command = 'slap';
      this.parseType = 'startsWith';
      return this.rankPrivelege = 'mod';
    };

    slapCommand.prototype.getSlapaction = function(username) {
      var p, slapaction, slaps;
      slaps = ["/me slaps @{victim}"];
      p = Math.floor(Math.random() * slaps.length);
      slapaction = slaps[p].replace('{victim}', username);
      return slapaction;
    };

    slapCommand.prototype.functionality = function() {
      var msg, name, r, user;
      msg = this.msgData.message;
      r = new RoomHelper();
      if (msg.length > 4) {
        ...
        if (user === false) {
          API.sendChat("/me slaps @" + this.msgData.from + " for getting the syntax wrong.");
          return setTimeout(function() {
            return API.sendChat("Seriously though, I don't recognize the username '" + name + "'");
          }, 750);
        } else {
          if (user.owner) {
            return API.sendChat(this.getSlapaction(this.msgData.from));
          } else {
            return API.sendChat(this.getSlapaction(user.username));
          }
        }
      }
    };
