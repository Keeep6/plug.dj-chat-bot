...
(function() {
  var Command, RoomHelper, User, afksCommand, allAfksCommand, announceCurate, antispam, apiHooks, avgVoteRatioCommand, badQualityCommand, beggar, chatCommandDispatcher, chatUniversals, cmdHelpCommand, cmds, commandsCommand, complimentCommand, cookieCommand, data, dieCommand, disconnectLookupCommand, downloadCommand, fbCommand, forceSkipCommand, handleNewSong, handleUserJoin, handleUserLeave, handleVote, hook, hugCommand, icecreamCommand, initEnvironment, initHooks, initialize, lockCommand, msToStr, overplayedCommand, popCommand, populateUserData, protectCommand, punishCommand, pupOnline, pushCommand, reloadCommand, resetAfkCommand, roomHelpCommand, rulesCommand, settings, skipCommand, sourceCommand, statusCommand, swapCommand, tacoCommand, themeCommand, undoHooks, unhook, unhookCommand, unlockCommand, updateVotes, voteRatioCommand, whyMehCommand, whyWootCommand, wootCommand,
      ...
      if (msg.length > 9) {
        ...
        switch (param) {
          ...
          case "rapes pup":
            ...
            break;
          case "icecream":
            resp = "order some delious icecream.  simply say 'icecream' or give some to someone else by saying 'icecream @user'";
            break;
          case "taco":
  ...
  icecreamCommand = (function(_super) {

    __extends(icecreamCommand, _super);

    function icecreamCommand() {
      return icecreamCommand.__super__.constructor.apply(this, arguments);
    }
    ...
    icecreamCommand.prototype.randomIcecream = function() {
      var r, icecream;
      icecreams = ["Strawberry Sunday", "Chocolate Chip Icecream Cone", "Cookie Dough Triple Scoop ", "Mint Chocolate Chip Icecream Cone", "Chocolate Icecream Sunday", "Banana Split with Whipped Cream", "Vanilla Icecream Cone with Sprinkles ", "Bubblegum Flavored Popcicle"];
      r = Math.floor(Math.random() * icecreams.length);
      return icecreams[r];
    };

    icecreamCommand.prototype.functionality = function() {
      var msg, icecream, icecreamName;
      msg = this.msgData.message;
      ...
      if (msg.substring(5, 6) === "@") {
        ...
        return API.sendChat("Yo @" + this.msgData.from + ", here is your " + icecream + "!");
      }
    };

    return icecreamCommand;
  ...
  complimentCommand = (function(_super) {

    __extends(complimentCommand, _super);

    function complimentCommand() {
      return complimentCommand.__super__.constructor.apply(this, arguments);
    }

    complimentCommand.prototype.init = function() {
      this.command = 'compliment';
      this.parseType = 'startsWith';
      return this.rankPrivelege = 'mod';
    };
    ...
    complimentCommand.prototype.functionality = function() {
      var msg, name, r, user;
      msg = this.msgData.message;
      r = new RoomHelper();
      if (msg.length > 11) {
        ...
        if (user === false) {
          ...
          if (user.owner) {
            return API.sendChat(this.getCompliment(this.msgData.from));
          } else {
            return API.sendChat(this.getCompliment(user.username));
          }
        }
      }
    };
  ...
  cmds = [hugCommand, tacoCommand, cookieCommand, icecreamCommand, punishCommand, complimentCommand, whyWootCommand, themeCommand, rulesCommand, roomHelpCommand, sourceCommand, wootCommand, badQualityCommand, downloadCommand, afksCommand, allAfksCommand, statusCommand, unhookCommand, dieCommand, reloadCommand, lockCommand, unlockCommand, swapCommand, popCommand, pushCommand, overplayedCommand, whyMehCommand, skipCommand, commandsCommand, resetAfkCommand, forceSkipCommand, fbCommand, cmdHelpCommand, protectCommand, disconnectLookupCommand, voteRatioCommand, avgVoteRatioCommand];
    ...
    if (plugRoomLinkPatt.exec(chat.message)) {
      ...
      if (!sender.ambassador && !sender.moderator && !sender.owner && !sender.superuser) {
        if (!data.users[chat.fromID]["protected"]) {
          API.sendChat("Don't spam room links please.");
          return API.moderateDeleteChat(chat.chatID);
        } else {
          return API.sendChat("I'm supposed to kick you, but i'll give you a warning.");
        }
      }
    }
  };
